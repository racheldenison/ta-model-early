function [decision evidence] = rd_temporalAttentionModel2(soa, endoCond)
%
...
% endo type
endoType = 'gamma'; % options: 'Gaussian', 'SSHat', 'gamma'

% Ex kernel shape
ExShape = 'exp'; % options: 'Gaussian', 'square', 'exp'

% proporational allocation of attention (based on cue validity)
% to make all or none, set prop = [1 0]
prop = [.7 .3]; % [.6 .4] % [high low], or [valid invalid]
...
integratorType = '1-stage';
ceiling = 1.6; % 2.3 (limited) % 1.6 (original+extended) % 375 (late competition)
...
%     figure
    ...
    cla
    hold on
    plot(x, 15.5 + stimulus,'k')
    plot(x, 13 + attnGainX - ior, 'Color', [153 124 107]/255)
    plot(x, 11 + EndoGain, 'Color', [185 178 170]/255)
    plot(x, 8 + attnGain, 'Color', [235 114 96]/255)
    plot(x, 6 + Eraw, 'Color', [112 191 65]/255)
